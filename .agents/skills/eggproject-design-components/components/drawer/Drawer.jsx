...
/* ============ Drawer ============
   Props:
     ...
     - size:     'sm' | 'md' (default) | 'lg' | 'xl'  — only for right/left
     - title, eyebrow, sub
     - footer:   ReactNode
...
function Drawer({
  ...
  // Append the sub (internal description) id to any consumer aria-describedby; dedupe, undefined when empty.
  const describedByTokens = `${ariaDescribedBy ?? ''} ${sub ? descId : ''}`.trim().split(/\s+/).filter(Boolean);
  ...
  React.useEffect(() => {
    ...
    // Register as the top of the shared modality stack. Drawer has two sibling roots
    // (separate overlay + panel) — both stay active; everything else is isolated. The
    // panel sits one visual layer above its own backdrop (layerOffset 0 → 1).
    if (manager) manager.register({
      id: entryId,
      roots: [
        { element: overlayRef.current, layerOffset: 0 },
        { element: panelRef.current, layerOffset: 1 },
      ],
    });

    // Initial focus, after the panel is laid out: explicit ref → first tabbable → panel.
  ...
  return (
    <>
      <div ref={overlayRef} className="drawer-overlay" onClick={closeOnOverlay ? (() => {
        const manager = getDialogManager();
        if (manager && !manager.isTopMost(entryId)) return;   // only the topmost backdrop closes
        onClose?.();
      }) : undefined}/>
      <aside ref={panelRef} tabIndex={-1} className={`drawer drawer--${side} drawer--${size}`} role="dialog" aria-modal="true" aria-labelledby={labelledBy} aria-label={ariaLabel} aria-describedby={describedBy}>
        {(title || eyebrow) && (
          <header className="drawer__header">
            <div className="drawer__header-text">
              {eyebrow && <span className="drawer__eyebrow">{eyebrow}</span>}
              {title && <h2 className="drawer__title" id={titleId}>{title}</h2>}
              {sub && <p className="drawer__subtitle" id={descId}>{sub}</p>}
            </div>
            {onClose && (
