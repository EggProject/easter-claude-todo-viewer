...
/* ============ Modal ============
   Composition:
     <Modal open onClose title eyebrow size>
       body content
       <ModalFooter>
         <Button .../>
       </ModalFooter>
   ...
   Props:
     - open:     boolean
     - onClose:  () => void  (Esc + overlay click + close button)
     - title:    string | ReactNode
     - eyebrow:  string — small uppercase label above title
     - sub:      string — secondary line below title
     - size:     'sm' | 'md' (default) | 'lg' | 'xl'
     - icon:     ReactNode — small leading icon
     - iconVariant: 'info' (default) | 'success' | 'warning' | 'danger'
     - footer:   ReactNode — buttons / split actions
...
function Modal({
  ...
  // Consumer aria-labelledby wins; else the generated title id when a title renders. Never auto-build from title text.
  const labelledBy = ariaLabelledBy || (title ? titleId : undefined);
  // Append the subtitle (internal description) id to any consumer aria-describedby; dedupe, no stray whitespace, undefined when empty.
  const describedByTokens = `${ariaDescribedBy ?? ''} ${subtitle ? descId : ''}`.trim().split(/\s+/).filter(Boolean);
  ...
  React.useEffect(() => {
    ...
    // lock, syncs the visual layer). Single root: backdrop + panel share .modal-overlay (layerOffset 0).
  ...
  return (
    <div
      ...
      onClick={(event) => {
        if (!closeOnOverlay || event.target !== event.currentTarget) return;
        const manager = getDialogManager();
    ...
    >
      ...
      >
        <div className="modal__header">
          {icon && (
            <span className={`modal__icon modal__icon--${iconVariant}`} aria-hidden="true">{icon}</span>
          )}
          <div className="modal__header-text">
            {eyebrow && <span className="modal__eyebrow">{eyebrow}</span>}
            {title && <h2 className="modal__title" id={titleId}>{title}</h2>}
            {subtitle && <p className="modal__subtitle" id={descId}>{subtitle}</p>}
          </div>
          {onClose && (
...
function ModalFooter({ split, left, children }) {
  if (split) return (
...
Object.assign(window, { Modal, ModalFooter });
