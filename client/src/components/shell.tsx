...
export function Shell({ children }: { children?: ReactNode }): ReactElement {
  ...
  const navClass = ({ isActive }: { isActive: boolean }) =>
    `app-side__link ${isActive ? 'is-on' : ''}`;
  const tooltip = sessionTooltip(
    currentSession,
    a.sessionsState ?? { sessions: [], currentSessionId: null, watchedSessionIds: [] },
  );
      ...
      <aside className="app-side">
        ...
        <div className="app-side__group">
          ...
          <NavLink to="/sessions" className={navClass}>
            ...
            >
              <path d="M4 19V5a2 2 0 0 1 2-2h13.4a.5.5 0 0 1 .4.8l-3.1 3.1c-.1.2-.2.5-.2.8v11.5a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z" />
            </svg>
            Sessions
          </NavLink>
        ...
        <header className="app-top">
          ...
          <div className="app-top__actions">
            ...
            <Link to="/sessions" className="btn btn--secondary btn--sm" title={tooltip}>
              ...
              {currentSession
                ? currentSession.label || shortId(currentSession.id)
                : 'Select session'}
            </Link>
            <button
              className="btn btn--secondary btn--sm"
              onClick={() => a.setSidebar?.(true)}
              title="Common change history"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
