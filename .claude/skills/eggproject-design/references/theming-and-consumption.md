...
How another skill **inside this repo** references the EggProject tokens, and how
the light/dark runtime works. Building in a project *outside* this repo? You copy
the token layer in instead — see "Taking the tokens into your own project" in
`SKILL.md`; the path math below does not apply to you. For the token catalog see
`token-reference.md`; for principles see `SKILL.md`.

## Contents
- [Import, never copy (skill-to-skill)](#import-never-copy-skill-to-skill)
...
## Import, never copy (skill-to-skill)

There is **no formal cross-skill dependency / `depends_on` mechanism** for skills
— referencing the tokens is just a filesystem reference along a relative path.
The iron rule of this family: **import `colors_and_type.css` from this skill's
folder; never copy it into another skill.** This governs edits *within this repo*
— an outside project copies the token layer in and owns that copy.
...
| Sibling-skill file | Depth | Relative path to tokens |
|---|---|---|
...
document. So no matter how deep the referencing file is, once it has loaded
`colors_and_type.css` the browser fetches `assets/fonts/fonts.css` (and the
`.woff2`) from **this** skill's folder automatically. Nothing links the fonts
itself, and there is no Google Fonts CDN anywhere. The same holds after you copy
the layer into your own project — copy `assets/fonts/` along with the CSS and the
self-resolving `@import` keeps working.
...
and lives here. Never redefine an `--ep-` token in another skill of this family —
that's the one thing that breaks the single-source guarantee across the family. A
project that copied the token layer in has its own single source, in that copy.
