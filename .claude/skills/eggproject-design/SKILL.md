...
concrete components**. Every other `eggproject-design-*` skill in this repo
imports the tokens from here rather than duplicating them.
...
work from*, not a library you link to. Copy what you need into the target project
(or re-implement from it) and rewrite every `../eggproject-*/…` path to that
project's own layout — a cross-skill relative path left in an outside project
resolves to nothing. For the tokens, copy the **whole theme layer** as one unit;
see "Taking the tokens into your own project" below.
...
Every "never copy" rule in this file is scoped to that second mode.
...
| `DESIGN.md` | The brand's full ground truth in the **Google Labs DESIGN.md format** — YAML token frontmatter + the six fixed sections. The worked example to trim down when writing a `DESIGN.md` into a target project. **The canonical file lives here**; the repo root only symlinks to it. |
| `PRODUCT.md` | Register, users, purpose, brand personality, anti-references, design principles, accessibility stance. The companion `/impeccable` reads alongside `DESIGN.md`. **Canonical here**, symlinked from the repo root. |
...
## Taking the tokens into your own project

Copy the **whole theme layer** as one unit — the light/dark scopes and the WCAG
tuning were verified together, so never cherry-pick individual values out of it:

```
colors_and_type.css   the barrel
tokens/               every module it chains
assets/fonts/         the .woff2 files + fonts.css
preview/_theme.js     only if you want the static theme toggle
```

Keep those files' positions relative to each other and the internal `@import`s
keep working unchanged. That copy is then **your** single source in that project;
refresh it from here rather than hand-editing it into drift.

Then write a **`DESIGN.md`** at the target project root — YAML token frontmatter
plus `Overview / Colors / Typography / Elevation / Components / Do's and Don'ts`,
covering only what you actually shipped. Later agent sessions in that project then
stay on brand without this skill installed, and `/impeccable` picks it up: its
setup reads `PRODUCT.md` + `DESIGN.md` from the project root and forces an `init`
when they are missing.

**The templates ship with this skill.** `DESIGN.md` and `PRODUCT.md` sit at this
skill's root — read them as the worked example of both formats, then write a
*trimmed* version into the target project. Do not copy this one wholesale: it
documents all 52 components of the full system, and a portable design-context file
is paid for on every load. Carry over only the tokens and components that project
actually uses.

## How sibling skills reference these tokens (repo-internal)

Applies to the `eggproject-design-*` skills in this repo only. There is **no
formal cross-skill dependency API** — it is a plain filesystem reference. The
rule inside the family: **import `colors_and_type.css` from this skill's folder;
never duplicate it into another skill folder.** One source keeps light/dark and
contrast correct across all eight skills.

Skills install as **siblings** under `.../skills/`, so a sibling reaches this
skill with a relative path that climbs to the `skills/` parent and descends into
`eggproject-design/`. The hop count = (depth of the referencing file below its
own skill root) + 1:

| Sibling-skill file (depth below its own skill root) | Path to the tokens |
|---|---|
...
These paths resolve only in this repo's sibling layout; an outside project has no
such layout, which is why it copies instead. Serve the **`skills/` parent as the
HTTP docroot** when testing here, so every `eggproject-<skill>/…` path resolves
deterministically. The fonts ride along automatically: `colors_and_type.css`
imports `assets/fonts/fonts.css` relative to *itself*, so however deep the
referencing file is, the fonts resolve. Full detail and the `_theme.js` wiring
are in `references/theming-and-consumption.md`.
...
local. Never redefine an `--ep-` token in another skill of this family — that is
what breaks the single-source guarantee. (In your own project, your copy of the
token file is that project's single source; edit it there, in one place.)
